@if (Auth::user()->user_role != 'admin')
	<script>window.location = '/menu'</script>
@endif

@extends('layouts.app')

@section('title', 'Student Requests')

@section('action-form')

	@foreach ($wagons as $wagon)

		<div class="card mb-2">
			
			<div class="card-header">

				<a href='#collapse-{{ $wagon["id"] }}' class="card-link" data-toggle="collapse">
					{{ $wagon->date_borrowed }} {{ $wagon->user->name }}
				</a>

			</div>

		</div>

		<div class="collapse" id='collapse-{{ $wagon["id"] }}' data-parent="#accordion">
			
			<div class="card-body">
				
				<table class="table table-bordered">
					
					<thead>
						
						<tr>
							
							<th>Date Borrowed</th>
							<th>Student Name</th>	
							<th>Item Name</th>		
							<th>Category</th>		
							<th>Status</th>		
							<th>Action</th>		

						</tr>	

						<tbody>

							<tr>
								
								<td class="text-center">{{ $wagon->date_borrowed }}</td>
								<td class="text-center">{{ $wagon->user->name}}</td>
								<td class="text-center">{{ $wagon->item->name}}</td>
								<td class="text-center">{{ $wagon->category->name }}</td>
								<td class="text-center">{{ $wagon->status->item_status }}</td>
								<td class="text-center">

									{{ $wagon->action->action }}
			
									<div class="text-center btn-group btn-block">

										<form action='{{ url("action/$wagon->id/accept") }}' method="post">
											@csrf

											@method("PUT")
											<input type="hidden" value="{{$wagon->item_id}}" name="itemId">
											<button class="btn btn-success">Approve</button>
										</form>

										<form action='{{ url("action/$wagon->id/deny") }}' method="post" enctype="multipart/form-data">
											@csrf

											@method("PUT")
											<input type="hidden" value="{{$wagon->item_id}}" name="itemId">
											<button class="btn btn-danger">Deny</button>
										</form>

									</div>

								</td>

							</tr>	

						</tbody>					

					</thead>

				</table>

			</div>

		</div>

	@endforeach

@endsection

@section('content')

	<div class="container-fluid">
		
		<h3>Student Requests</h3>

		<div id="accordion">
			
			@yield('action-form')

		</div>

	</div>

@endsection
